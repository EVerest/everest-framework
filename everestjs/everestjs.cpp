...
struct EvModCtx {
    ...
    std::map<std::tuple<std::string, uint64_t, std::string>, Napi::FunctionReference> var_subscriptions;
...
static Napi::Value set_var_subscription_handler(const std::string& requirement_id, uint64_t index,
                                                const std::string& var_name, const Napi::CallbackInfo& info) {
    BOOST_LOG_FUNCTION();
    const auto& env = info.Env();
    ...
    try {
        ...
        auto sub_key = std::make_tuple(requirement_id, index, var_name);
        ...
        // FIXME (aw): in principle we could also pass this reference down to js_cb
        ctx->everest.subscribe_var(requirement_id, index, var_name, [sub_key](Everest::json input) {
            ctx->js_cb->exec(
...
static Napi::Value call_cmd(const std::string& requirement_id, uint64_t index, const std::string& cmd_name,
                            const Napi::CallbackInfo& info) {
    BOOST_LOG_FUNCTION();
    ...
    try {
        ...
        const auto& retval = ctx->everest.call_cmd(requirement_id, index, cmd_name, argument);
...
static Napi::Value boot_module(const Napi::CallbackInfo& info) {
    ...
    try {
        ...
        for (auto& requirement : module_manifest["requires"].items()) {
            ...
            for (uint64_t i = 0; i < req_route_list.size(); i++) {
                ...
                auto requirement_impl_intf =
                    config->get_interface_definition(interface_name);
