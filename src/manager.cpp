...
static SubprocessHandle exec_python_module(const ModuleStartInfo& module_info, const RuntimeSettings& rs) {
    ...
    auto pythonpath = rs.main_dir / "everestpy";
...
static std::map<pid_t, std::string> start_modules(Config& config, MQTTAbstraction& mqtt_abstraction,
    ...
    for (const auto& module : main_config.items()) {
        ...
        boost::filesystem::path python_module_path = module_path / python_filename;
        ...
        if (boost::filesystem::exists(binary_path)) {
        ...
        } else if (boost::filesystem::exists(python_module_path)) {
            ...
            modules_to_spawn.emplace_back(module_name, printable_module_name, ModuleStartInfo::Language::python,
                                          boost::filesystem::canonical(python_module_path));
        } else {
