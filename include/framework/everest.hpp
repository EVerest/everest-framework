...
private:
    ...
    /// \p error_type. The given \p raise_callback is called when a new error is raised and \p clear_callback is called when
    /// an error is cleared
    ///
    ...
    ///
    /// \brief Subscribes globally to all errors of all modules. The given \p raise_callback is called when a new error is
    /// raised. The given \p clear_callback is called when an error is cleared
    ///
    void subscribe_global_all_errors(const error::ErrorCallback& raise_callback, const error::ErrorCallback& clear_callback);
