...
json get_module_config(const MQTTSettings& mqtt_settings, const std::string& module_id) {
    ...
    auto get_config_topic = fmt::format("{}modules/{}/get_config", mqtt_settings.everest_prefix, module_id);
    auto config_topic = fmt::format("{}modules/{}/config", mqtt_settings.everest_prefix, module_id);
    ...
    auto interface_names_topic = fmt::format("{}interfaces", mqtt_settings.everest_prefix);
    ...
    for (auto& interface : interface_names) {
        auto interface_topic =
            fmt::format("{}interface_definitions/{}", mqtt_settings.everest_prefix, interface.get<std::string>());
    ...
    auto types_topic = fmt::format("{}types", mqtt_settings.everest_prefix);
    result["types"] = mqtt.get(types_topic, QOS::QOS2);

    auto module_provides_topic = fmt::format("{}module_provides", mqtt_settings.everest_prefix);
    auto module_provides = mqtt.get(module_provides_topic, QOS::QOS2);
    result["module_provides"] = module_provides;

    auto settings_topic = fmt::format("{}settings", mqtt_settings.everest_prefix);
    auto settings = mqtt.get(settings_topic, QOS::QOS2);
    result["settings"] = settings;

    auto schemas_topic = fmt::format("{}schemas", mqtt_settings.everest_prefix);
    auto schemas = mqtt.get(schemas_topic, QOS::QOS2);
    result["schemas"] = schemas;

    auto manifests_topic = fmt::format("{}manifests", mqtt_settings.everest_prefix);
    auto manifests = mqtt.get(manifests_topic, QOS::QOS2);
    result["manifests"] = manifests;

    auto error_types_map_topic = fmt::format("{}error_types_map", mqtt_settings.everest_prefix);
    auto error_types_map = mqtt.get(error_types_map_topic, QOS::QOS2);
    result["error_map"] = error_types_map;

    auto module_config_cache_topic = fmt::format("{}module_config_cache", mqtt_settings.everest_prefix);
