...
ManagerSettings::ManagerSettings(const std::string& prefix_, const std::string& config_, const std::string& db_) {
    ...
    if (db_.length() != 0) {
        // user provided database file
        try {
            db_dir = assert_file(db_, "User provided database", MissingFilePolicy::Create);
        } catch (const BootException& e) {
            db_dir = data_dir / defaults::DEFAULT_DATABASE_FILE_NAME;
        }
    } else {
        // default database file
        db_dir = data_dir / defaults::DEFAULT_DATABASE_FILE_NAME;
    }
    // now the db_dir should have been found

    const auto user_selected_config_source = db_.empty() ? ConfigBootSource::YamlFile : ConfigBootSource::Database;
    const auto migrations_dir = data_dir / "migrations";
    everest::config::SqliteStorage storage(db_dir);
    ...
    if (user_selected_config_source == ConfigBootSource::Database && storage_marked_valid) {
        boot_source = ConfigBootSource::Database;
        ...
        boot_source = ConfigBootSource::YamlFile;
    }

    everest::config::Settings settings;
    if (boot_source == ConfigBootSource::YamlFile) {
