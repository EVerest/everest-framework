...
struct EverestContext {
    EverestContext(Everest::Everest& everest) : everest(everest) {
    }
...
struct EverestPy {
    ...
    std::function<void(const Reqs& reqs, const std::function<void()>& request_shutdown)> pre_init;
    std::function<std::vector<EverestPyCmd>(const json& connections)> everest_register;
    std::unique_ptr<EverestContext> context;
    ...
    int initialize(const std::string& prefix, const std::string& config_file, const std::string& module_id) {
        ...
        try {
            ...
            context = std::make_unique<EverestContext>(Everest::Everest::get_instance(
                module_id, config, rs.validate_schema, rs.mqtt_broker_host, rs.mqtt_broker_port, rs.mqtt_everest_prefix,
            ...
            for (const auto& impl_definition : module_impls.items()) {
                ...
                if (impl_intf.contains("vars")) {
                    for (const auto& var_entry : impl_intf["vars"].items()) {
                        ...
                        reqs.pub_vars[impl_id][var_name] = [&everest = context->everest, impl_id,
                                                            var_name](json json_value) {
                ...
                for (size_t i = 0; i < req_route_list.size(); i++) {
                    ...
                    for (auto var_name : requirement_vars) {
                        reqs.vars[requirement_id][requirement_module_id][var_name] =
                            [&everest = context->everest, requirement_id, i,
                    ...
                    for (auto const& cmd_name : requirement_cmds) {
                        reqs.call_cmds[requirement_id][requirement_module_id][cmd_name] = {
                            [&everest = context->everest, requirement_id, i, cmd_name](json parameters) {
            ...
            module_adapter.call = [&everest = context->everest](const Requirement& req, const std::string& cmd_name,
                                                                Parameters args) {
                return everest.call_cmd(req, cmd_name, args);
            };

            module_adapter.publish = [&everest = context->everest](const std::string& param1, const std::string& param2,
                                                                   Value param3) {
                return everest.publish_var(param1, param2, param3);
            };

            module_adapter.subscribe = [&everest = context->everest](const Requirement& req,
            ...
            module_adapter.ext_mqtt_publish = std::bind(&::Everest::Everest::external_mqtt_publish, &context->everest,
                                                        std::placeholders::_1, std::placeholders::_2);

            // NOLINTNEXTLINE(modernize-avoid-bind): prefer bind here for readability
            module_adapter.ext_mqtt_subscribe =
                std::bind(&::Everest::Everest::provide_external_mqtt_handler, &context->everest, std::placeholders::_1,
                          std::placeholders::_2);

            module_callbacks.register_module_adapter(module_adapter);

            pre_init(reqs, [&everest = context->everest]() { everest.signal_shutdown(); });

            // FIXME (aw): would be nice to move this config related thing toward the module_init function
            std::vector<EverestPyCmd> cmds = everest_register(config.get_main_config()[module_id]["connections"]);

            for (auto const& command : cmds) {
                context->everest.provide_cmd(command.impl_id, command.cmd_name, command.handler);
            }
            ...
            module_info.telemetry_enabled = context->everest.is_telemetry_enabled();

            module_callbacks.init(module_configs, module_info);

            context->everest.spawn_main_loop_thread();

            // register the modules ready handler with the framework
            // this handler gets called when the global ready signal is received
            context->everest.register_on_ready_handler(module_callbacks.ready);
            ...
            context->everest.register_on_shutdown_handler(module_callbacks.shutdown);

            // the module should now be ready
            context->everest.signal_ready();
    ...
    void register_module_adapter_callback(
        ...
        module_callbacks.register_module_adapter = register_module_adapter;
    }
    ...
    void register_everest_register_callback(
        ...
        this->everest_register = everest_register;
    }

    void register_init_callback(const std::function<void(ModuleConfigs module_configs, const ModuleInfo& info)>& init) {
        module_callbacks.init = init;
    }

    void register_pre_init_callback(
        std::function<void(const Reqs& reqs, const std::function<void()>& request_shutdown)>& pre_init) {
        this->pre_init = pre_init;
    }

    void register_ready_callback(const std::function<void()>& ready) {
        module_callbacks.ready = ready;
    }

    void register_shutdown_callback(const std::function<void()>& shutdown) {
        module_callbacks.shutdown = shutdown;
    }

    void shutdown() {
        context->everest.disconnect();
        context->everest.wait_for_main_loop_end();
        _exit(EXIT_SUCCESS);
    }
};
...
PYBIND11_MODULE(everestpy, m) {
    ...
    py::class_<EverestPy>(m, "EverestPy")
        .def(py::init<>())
        .def("init", &EverestPy::initialize)
        .def("register_module_adapter_callback", &EverestPy::register_module_adapter_callback)
        .def("register_everest_register_callback", &EverestPy::register_everest_register_callback)
        .def("register_init_callback", &EverestPy::register_init_callback)
        .def("register_pre_init_callback", &EverestPy::register_pre_init_callback)
        .def("register_ready_callback", &EverestPy::register_ready_callback)
        .def("register_shutdown_callback", &EverestPy::register_shutdown_callback)
        .def("shutdown", &EverestPy::shutdown);
